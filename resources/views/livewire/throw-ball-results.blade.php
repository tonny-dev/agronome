<div>
    ...
    <div class="bg-white shadow rounded-lg border border-b-2 w-auto lg:w-4/5 m-auto">
        ...
        <h2 class="text-center underline">Ball Throw Test Results</h2>
        <div class="grid grid-cols-2 gap-0 ml-8 mt-4">
            ...
            <p class="font-thin">Ball Throw Test</p>

            <p class="font-medium">Location</p>
            <p class="font-thin" id="p_location">{{$field_details}}</p>

            <p class="font-medium">Test Date:</p>
            <p id="p_date" class="font-thin">{{$test_date}}</p>

            <p class="font-medium">Test Purpose :</p>
            <p id="" class="font-thin">To check the possible presence of sand or clay</p>
        ...
        <div class="flex justify-between mx-4 mb-2">
            <button id="btn_tb_delete" class="border border-red-500 pl-2 pr-2 rounded text-red-500">Delete</button>
            <button wire:ignore id="btn_finish_refresh" class="border border-green-500 pl-2 pr-2 rounded text-green-500">Confirm</button>
        </div>
    </div>


    <script>
        $('#btn_finish_refresh').off().on('click', function() {

            Livewire.emit(
                'throw_ball_test_completed',
            ...
            div_toggle('div_physical_tests', 'div_throw_ball_results');


        });

        $('#btn_tb_delete').off().on('click', function() {
            swal({
                    title: "Success",
