<div>
    ...
    <div class="bg-white shadow rounded-lg border border-b-2 w-auto lg:w-4/5 m-auto">
        <h2 class="text-center underline">Squeeze Ball Test Results</h2>
        ...
        <div class="grid grid-cols-2 gap-0 ml-8 mt-4">
            ...
            <span class="font-thin"> Squeeze Ball Test</span>

            <span class="font-medium">Location</span>
            <span class="font-thin" id="squeeze_ball_span_location">{{$field_details}}</span>

            <span class="font-medium">Test Date:</span>
            <span id="squeeze_ball_span_date" class="font-thin">{{$test_date}}</span>

            <span class="font-medium">Test Purpose :</span>
            <span id="" class="font-thin">To test the presence of clay in the soil</span>
        ...
        <div class="flex justify-between mx-4 mb-2">
            <button id="btn_squeez_delete" class="border border-red-500 pl-2 pr-2 rounded text-red-500">Delete</button>
            <button wire:ignore id="btn_finish_squeeze_ball" class="border border-green-500 pl-2 pr-2 rounded text-green-500">Confirm</button>
        </div>
    </div>


    <script>
        $('#btn_finish_squeeze_ball').off().on('click', function() {

            Livewire.emit(
                'squeeze_ball_test_completed',
            ...
            div_toggle('div_physical_tests', 'div_squeeze_ball_results');

        });

        $('#btn_squeez_delete').off().on('click', function() {
            swal({
                title: "Success",
