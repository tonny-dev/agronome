<div class="bg-white shadow rounded-lg border border-b-2">
    ...
    <div class="grid grid-cols-2 gap-0 mx-2 mt-2">
        ...
        <span class="font-thin" id="squeeze_ball_span_location"> </span>

        <span class="font-medium">Test Date:</span>
        <span id="squeeze_ball_span_date" class="font-thin"></span>
    ...
    <div class="flex justify-between mx-4 mb-2">
        <button id="btn_squeeze_back" class="border border-green-500 px-2 rounded">Back</button>
        <button id="btn_squeeze_ball_throw_proceed" class="btn_proceed_summary">Start</button>
    </div>
</div>


<script>
    $('#btn_squeeze_ball_throw_proceed').on('click', function() {
        $("#testTipo").css({
            "color": "#374151",
        });
        ...
        div_toggle('div_squeeze_ball_test', 'div_squeeze_ball_summary');


    });


    $('#btn_squeeze_back').on('click', function() {
        div_toggle('div_physical_tests','div_squeeze_ball_summary');
